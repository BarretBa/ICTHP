...
def main(cfg: TrainerConfig) -> None:
    ...
    run_name = time_name + f"pick_high_{os.path.basename(cfg.model.pretrained_model_name_or_path)}"
    ...
    logger.info(f"Loading optimizer:{cfg.optimizer}")
    ...
    for epoch in range(accelerator.cfg.num_epochs):
        ...
        for step, batch in enumerate(split2dataloader[cfg.dataset.train_split_name]):
            ...
            if accelerator.should_eval():
                ...
                evaluate()
                toc=time.time()
                print(f"eval time: {toc-tic}")
            
            if (accelerator.global_step % 10 == 0):
                accelerator.unwrap_and_save_step(model)

            if accelerator.should_save():
                if (accelerator.global_step % 5000 == 0):
                    accelerator.unwrap_and_save_step(model)

            model.train()

            with accelerator.accumulate(model):
                loss ,hp_loss,icthp_loss = task.train_step(model, criterion, batch,accelerator.global_step)
                avg_loss = accelerator.gather(loss).mean().item()
                avg_hp_loss = accelerator.gather(hp_loss).mean().item()
                avg_icthp_loss = accelerator.gather(icthp_loss).mean().item()
            ...
            accelerator.update_step(avg_loss, lr, avg_hp_loss, avg_icthp_loss)
